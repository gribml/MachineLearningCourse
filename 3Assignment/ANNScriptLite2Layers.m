        ...
        for r = 1:length(lr)
            ...
            for i=minneurons:neurIncr:maxneurons
                for j = minneurons:neurIncr:maxneurons
                    ...
                    net = buildNetwork([i,j], nepochs, [0.667, 0.33, 0], x2, y2, perfFun, lr(r), transFun{tf}, trainFun{tn});
                    pred = testANN(net, x2);
...
%                     fprintf(FID_single, 'for one 6-outputted NN: (%s, %f, %s, %s): %f', perfFun, lr(r), transFun{tf}, trainFun{tn});
                    ...
                    binaryNets = cell( l, 1 );
                    for k=1:l
                        binaryNets{k} = buildNetwork( [i, j], nepochs, [0.667, 0.33, 0], x2, y2(k,:), perfFun, lr(r), transFun{tf}, trainFun{tn} );        
                    end
...
%                     fprintf(FID_multi, 'for 6 1-outputted NN: (%s, %f, %s, %s): %f', perfFun, lr(r), transFun{tf}, trainFun{tn});
                    ...
                    multi.precision = pr;
                    multi.classRate = cr;
                    save(sprintf('results_2layers/perf_%s_%s_%f_%d-%d.mat', transFun{tf}, trainFun{tn}, lr(r), i,j), 'single', 'multi');
                end
            end
        end
    end
